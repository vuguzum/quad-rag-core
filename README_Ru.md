...
**Quad-RAG-Core** — middleware модуль на Python для систем семантического поиска по локальным папкам с векторной базой данных Qdrant

и автоматическим отслеживанием изменений файлов и поддержанием индекса.
...
🔌 **Адаптивность к фреймворкам** — легко интегрируется с MCP, FastAPI, CLI и другими протоколами
...
Три бэкенда для извлечения текста из PDF с автоматическим откатом: PyPDF2 → PyMuPDF → pdfplumber.

### Потокобезопасность
Используются блокировки потоков для безопасного конкурентного доступа к watcher'ам и отслеживанию прогресса.
